...
CREATE EXTENSION IF NOT EXISTS postgis; -- ============================================ -- 1. ENUMS -- ============================================ -- Users 
CREATE TYPE user_role AS ENUM ('CLIENT','COOK','ADMIN'); 
CREATE TYPE user_status AS ENUM 
('ACTIVE','SUSPENDED','DELETED','PENDING_VERIFICATION'); -- Cook 
CREATE TYPE cook_status AS ENUM 
('PENDING_APPROVAL','ACTIVE','PAUSED','SUSPENDED','REJECTED'); 
CREATE TYPE employment_status AS ENUM 
('AUTO_ENTREPRENEUR','PORTAGE_SALARIAL','MICRO_ENTREPRISE','ASSOCIA
 TION'); -- Booking 
CREATE TYPE booking_status AS ENUM 
('PENDING','ACCEPTED','CONFIRMED','IN_PROGRESS','COMPLETED','CANCELLE
 D','DISPUTED'); 
CREATE TYPE cancellation_reason AS ENUM 
('CLIENT_REQUEST','COOK_UNAVAILABLE','EMERGENCY','WEATHER','OTHER'); 
-- Day/Meal 
CREATE TYPE day_of_week AS ENUM 
('MONDAY','TUESDAY','WEDNESDAY','THURSDAY','FRIDAY','SATURDAY','SUND
 AY'); 
CREATE TYPE meal_type AS ENUM ('BREAKFAST','LUNCH','DINNER','BRUNCH'); -- Review / Rating 
CREATE TYPE message_type AS ENUM ('TEXT','IMAGE','SYSTEM'); -- Dispute 
CREATE TYPE dispute_status AS ENUM 
('OPEN','IN_REVIEW','RESOLVED','CLOSED'); 
CREATE TYPE dispute_reason AS ENUM 
('SERVICE_NOT_PROVIDED','POOR_QUALITY','LATE_ARRIVAL','HYGIENE_ISSUE'
 ,'DAMAGE','PAYMENT_ISSUE','OTHER'); -- Transaction 
CREATE TYPE transaction_type AS ENUM 
('BOOKING_PAYMENT','REFUND','PAYOUT','PLATFORM_FEE','BONUS','PENALTY'
 ); 
CREATE TYPE transaction_status AS ENUM 
('PENDING','COMPLETED','FAILED','CANCELLED'); -- Address 
CREATE TYPE address_type AS ENUM 
('HOME','VACATION_RENTAL','WORK','OTHER'); -- Notification 
CREATE TYPE notification_type AS ENUM ( 
'BOOKING_REQUEST','BOOKING_ACCEPTED','BOOKING_CONFIRMED','BOOKIN
 G_CANCELLED', 
'BOOKING_REMINDER','REVIEW_RECEIVED','MESSAGE_RECEIVED','PAYMENT_
 RECEIVED', 
'DISPUTE_OPENED','PROFILE_APPROVED','PROFILE_REJECTED','SYSTEM' 
); -- Promo 
CREATE TYPE promo_code_type AS ENUM 
('PERCENTAGE','FIXED_AMOUNT','FREE_SERVICE'); -- Cuisine / Dietary / Allergy / Specialty 
CREATE TYPE cuisine_type AS ENUM ( 
'FRENCH','ITALIAN','ASIAN','MEDITERRANEAN','VEGETARIAN','VEGAN','HEAL
 THY','TRADITIONAL', 
'GASTRONOMIC','COMFORT_FOOD','INTERNATIONAL','FUSION','SEAFOOD','GR
 ILLED','PASTRY' 
); 
CREATE TYPE dietary_restriction AS ENUM ( 
'VEGETARIAN','VEGAN','GLUTEN_FREE','LACTOSE_FREE','HALAL','KOSHER','LO
 W_CARB','KETO','PALEO','DIABETIC' 
); 
CREATE TYPE allergy AS ENUM ( 
'PEANUTS','TREE_NUTS','MILK','EGGS','FISH','SHELLFISH','SOY','WHEAT','SESAM
 E','MUSTARD','CELERY','LUPIN','SULFITES' 
); 
CREATE TYPE specialty AS ENUM ( 
'FRENCH_CUISINE','ITALIAN_PASTA','SUSHI','BARBECUE','PASTRY','BREAD_MA
 KING','MOLECULAR_CUISINE','RAW_FOOD', 
'BABY_FOOD','DIET_MEALS','PARTY_CATERING','BREAKFAST','BRUNCH','TAPAS
 ','FINGER_FOOD' 
); -- Certification 
CREATE TYPE certification_type AS ENUM 
('HYGIENE_HACCP','FIRST_AID','FOOD_ALLERGY','PROFESSIONAL_CHEF','NUTR
 ITION','PASTRY','WINE_PAIRING','OTHER'); -- ============================================ -- 2. USERS -- ============================================ 
CREATE TABLE users ( 
...
CREATE INDEX idx_users_role_status ON users(role, status); -- ============================================ -- 3. COOK PROFILE -- ============================================ 
...
CREATE INDEX idx_cook_profiles_rating ON cook_profiles(average_rating); -- ============================================ -- 4. CLIENT PROFILE -- ============================================ 
...
CREATE UNIQUE INDEX idx_blocked_dates_cook_date ON 
blocked_dates(cook_profile_id, date); -- ============================================ -- 6. BOOKINGS -- ============================================ 
...
CREATE INDEX idx_bookings_cook ON bookings(cook_profile_id); -- ============================================ -- 7. MESSAGERIE -- ============================================ 
...
CREATE INDEX idx_transactions_status ON transactions(status); -- ============================================ -- 9. ADRESSES -- ============================================ 
...
CREATE INDEX idx_reviews_reviewer ON reviews(reviewer_id); -- ============================================ -- 13. COOK PORTFOLIO / CERTIFICATIONS -- ============================================ 
...
CREATE INDEX idx_disputes_status ON disputes(status); -- ============================================ -- 15. PROMO CODES -- ============================================ 
...
CREATE UNIQUE INDEX idx_referrals_pair ON referrals(referrer_id, referee_id); -- ============================================ -- 17. AUDIT LOGS / ACTIVITY TRACKING -- ============================================ 
...
CREATE INDEX idx_audit_logs_table ON audit_logs(table_name, record_id); -- ============================================ -- 18. SYSTEM SETTINGS / CONFIG -- ============================================ 
...
created_at TIMESTAMP DEFAULT now(), 
updated_at TIMESTAMP DEFAULT now() 
); -- ============================================ -- 19. FINAL OPTIMISATIONS -- ============================================ -- Full Text Search (optionnel pour recherches cuisine, bio, etc.) 
CREATE INDEX idx_cook_profiles_bio_fts ON cook_profiles USING 
GIN(to_tsvector('french', bio)); -- Geospatial Index 
CREATE INDEX idx_users_location ON users USING GIST(location); 
CREATE INDEX idx_cook_profiles_location ON cook_profiles USING GIST(location); -- Trigger: updated_at 
