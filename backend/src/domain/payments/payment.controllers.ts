...
export const createPaymentIntent = async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    ...
    if (booking.payment_intent_id) {
      const existingIntent = await StripeService.getPaymentIntent(booking.payment_intent_id);
    ...
    const cookEarnings = StripeService.calculateCookEarnings(booking.total_price, platformFee);

    // Create payment intent
    const paymentIntent = await StripeService.createPaymentIntent(booking.total_price, 'eur', {
...
export const getPaymentIntent = async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    if (!req.user) {
      res.status(401).json({
        error: 'Unauthorized',
    ...
    const booking = await BookingStore.getBookingById(paymentIntent.metadata.booking_id || '');
    if (!booking) {
      res.status(404).json({
        error: 'Not Found',
        message: 'Booking not found',
...
export const getRefund = async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    if (!req.user) {
      res.status(401).json({
        error: 'Unauthorized',
