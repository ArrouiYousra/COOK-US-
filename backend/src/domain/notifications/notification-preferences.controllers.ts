  ...
  try {
    ...
    try {
      ...
      if (error && error.code !== "PGRST116") {
        console.error(
          "Erreur Supabase lors de la récupération des préférences:",
          error,
        );
        ...
        if (
          ...
          console.warn(
            "La colonne notification_preferences n'existe pas, utilisation des valeurs par défaut",
          );
          res.status(200).json({
        ...
        console.warn(
          "Erreur Supabase, utilisation des valeurs par défaut:",
          error.message,
        );
        res.status(200).json({
          preferences: DEFAULT_PREFERENCES,
      ...
      console.error(
        "Erreur lors de la récupération des préférences depuis la DB:",
        dbError,
      );
      res.status(200).json({
        preferences: DEFAULT_PREFERENCES,
...
export const updateNotificationPreferences = async (
  ...
  try {
    ...
    const currentPreferences =
      (currentData?.notification_preferences as NotificationPreferences) ||
      DEFAULT_PREFERENCES;
