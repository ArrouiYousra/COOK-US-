...
const MAX_FILE_SIZE_BYTES =
  (Number(process.env.RIB_MAX_FILE_SIZE_MB ?? "10") || 10) * 1024 * 1024;
...
const extractBase64Payload = (
  ...
  if (!ALLOWED_CONTENT_TYPES.has(contentType)) {
    throw new Error(
      "Format de fichier RIB non supporté. Formats acceptés: PDF, JPEG, PNG.",
    );
  }
  ...
  if (buffer.length > MAX_FILE_SIZE_BYTES) {
    throw new Error(
      `Le document RIB dépasse la taille maximale autorisée (${MAX_FILE_SIZE_BYTES / (1024 * 1024)} MB)`,
    );
  }
...
const ensureBucketConfigured = (): void => {
  if (!RIB_BUCKET) {
    throw new Error(
      "La variable SUPABASE_STORAGE_RIB_BUCKET doit être configurée pour l’upload RIB",
    );
  }
};
...
export class DocumentService {
  ...
  static async deleteRibDocument(filePath: string): Promise<void> {
    ...
    if (pathParts[0] !== RIB_BUCKET) {
      throw new Error(
        `Le chemin du fichier ne correspond pas au bucket ${RIB_BUCKET}`,
      );
    }
    ...
    if (error) {
      throw new Error(
        `Échec de la suppression du document RIB: ${error.message}`,
      );
    }
  }
}
