    ...
    if (booking) {
      ...
      if (clientProfile) {
        ...
        if (reservation.cook && reservation.cook.user) {
          const cookName =
            `${reservation.cook.user.first_name} ${reservation.cook.user.last_name}`.trim();
          await NotificationService.sendProposalReceivedNotification(
            ...
            console.error(
              "Failed to send proposal received notification:",
              err,
            ),
          );
        }
        ...
        await NotificationService.sendReservationCreatedNotification(
          req.user.id,
          {
          ...
          console.error(
            "Failed to send reservation created notification:",
            err,
          ),
        );
      }
 ...
 */
export const getReservationsByBookingId = async (
  req: AuthRequest,
  res: Response,
): Promise<void> => {
  try {
  ...
  try {
    ...
    if (booking && reservation.cook) {
      const bookingDate = booking.booking_date
        ? new Date(booking.booking_date).toLocaleDateString("fr-FR")
        : "date non spécifiée";
      const cookName = reservation.cook.user
        ? `${reservation.cook.user.first_name} ${reservation.cook.user.last_name}`.trim()
        : "Cuisinier";
      ...
      if (cookProfileData) {
        ...
        await NotificationService.sendReservationAcceptedNotification(
          ...
          console.error(
            "Failed to send reservation accepted notification:",
            err,
          ),
        );
    ...
    const errorMessage =
      error instanceof Error ? error.message : "Failed to accept reservation";

    // Gérer les erreurs spécifiques
    if (
      errorMessage.includes("introuvable") ||
      errorMessage.includes("ne peut plus")
    ) {
      res.status(400).json({
        error: "Bad Request",
        message: errorMessage,
    ...
    if (booking && reservation.cook) {
      ...
      if (cookProfileData) {
        await NotificationService.sendProposalRejectedNotification(
          ...
          console.error("Failed to send proposal rejected notification:", err),
        );
      }
    }
...
export const cancelReservation = async (
  ...
  try {
    ...
    if (booking && reservation.cook) {
      ...
      if (clientProfile) {
        await NotificationService.sendReservationCancelledNotification(
          clientProfile.user_id,
          {
            reservationId: reservation.id,
            bookingId: reservation.booking_id,
          ...
          console.error(
            "Failed to send reservation cancelled notification:",
            err,
          ),
        );
      }
