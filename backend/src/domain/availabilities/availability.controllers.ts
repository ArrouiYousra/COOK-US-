...
export const getMyAvailabilities = async (
  ...
  try {
    ...
    const availabilities = await AvailabilityStore.getAvailabilitiesByCook(
      cookProfile.id,
    );
...
export const getCookAvailabilities = async (
  req: AuthRequest,
  res: Response,
): Promise<void> => {
  try {
    ...
    const allAvailabilities = await AvailabilityStore.getAvailabilitiesByCook(
      cookProfile.id,
    );
...
export const createAvailability = async (
  ...
  try {
    ...
    const { day_of_week, meal_type, start_time, end_time, is_active } =
      req.body;
    ...
    if (!day_of_week || !meal_type || !start_time || !end_time) {
      res.status(400).json({
        error: "Bad Request",
        message:
          "day_of_week, meal_type, start_time, and end_time are required",
      });
      return;
    }

    const availability = await AvailabilityStore.createAvailability(
      cookProfile.id,
      {
        day_of_week,
        meal_type,
    ...
    const errorMessage =
      error instanceof Error ? error.message : "Failed to create availability";
...
export const deleteAvailability = async (
  ...
  try {
    ...
    const errorMessage =
      error instanceof Error ? error.message : "Failed to delete availability";
...
export const getMyBlockedDates = async (
  req: AuthRequest,
  res: Response,
): Promise<void> => {
  try {
    ...
    const blockedDates = await AvailabilityStore.getBlockedDatesByCook(
      cookProfile.id,
      {
...
export const createBlockedDate = async (
  ...
  try {
    ...
    const blockedDate = await AvailabilityStore.createBlockedDate(
      cookProfile.id,
      {
        date,
        reason,
...
export const deleteBlockedDate = async (
  req: AuthRequest,
  res: Response,
): Promise<void> => {
  try {
    ...
    const errorMessage =
      error instanceof Error ? error.message : "Failed to delete blocked date";
