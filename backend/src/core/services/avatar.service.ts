...
const ensureBucketConfigured = async (): Promise<void> => {
  if (!AVATAR_BUCKET) {
    throw new Error(
      "La variable SUPABASE_STORAGE_AVATAR_BUCKET doit être configurée pour l'upload d'avatar",
    );
  }
  ...
  try {
    ...
    const bucketExists = buckets?.some(
      (bucket) => bucket.name === AVATAR_BUCKET,
    );

    if (!bucketExists) {
      console.log(
        `Le bucket "${AVATAR_BUCKET}" n'existe pas, tentative de création...`,
      );
      ...
      if (createError) {
        ...
        throw new Error(
          `Impossible de créer le bucket "${AVATAR_BUCKET}": ${createError.message}. Veuillez créer le bucket manuellement dans Supabase Dashboard.`,
        );
      }
    ...
    throw new Error(
      `Erreur lors de la configuration du bucket "${AVATAR_BUCKET}": ${errorMessage}`,
    );
  }
};
