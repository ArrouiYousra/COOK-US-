...
export class BookingStore {
  ...
  static async createBooking(
    ...
    const hoursBooked = this.calculateHours(
    ...
    const subtotal =
      hoursBooked && hourlyRate ? hourlyRate * hoursBooked : null;
    ...
    const totalPrice =
      subtotal && platformFee
        ? subtotal + extraServicesPrice + platformFee
        : null;
    const cookEarnings =
      subtotal && platformFee
        ? subtotal + extraServicesPrice - platformFee
  ...
  static async getClientProposals(
    ...
    // Log pour déboguer
    console.log(
      `[BookingStore.getClientProposals] Client ${userId}: ${bookings.length} propositions privées trouvées`,
    );
   ...
   */
  static async getPublicRequests(filters?: {
    city?: string;
    limit?: number;
    offset?: number;
  ...
  static async updateBooking(
    ...
    if (
      isCompleting &&
      updatedBooking.cook_earnings &&
      updatedBooking.cook_earnings > 0
    ) {
      try {
        // Get cook user_id
        const { data: cookProfile } = await supabaseAdmin
  ...
  static async acceptBooking(
    ...
    if (booking.status === "PENDING") {
      ...
      throw new Error(
        "Can only accept bookings with PENDING or ACCEPTED status",
      );
    }
  ...
  static async cancelBooking(
    ...
    if (!["PENDING", "ACCEPTED", "CONFIRMED"].includes(booking.status)) {
      throw new Error(
        "Can only cancel bookings with PENDING, ACCEPTED, or CONFIRMED status",
      );
    }
