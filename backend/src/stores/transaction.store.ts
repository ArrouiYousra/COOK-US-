...
export class TransactionStore {
   ...
   */
  static async createTransaction(transactionData: CreateTransactionDTO): Promise<Transaction> {
    ...
    if (error) {
      ...
      throw new Error(`Failed to create transaction: ${error.message} (code: ${error.code})`);
    }
   ...
   */
  static async getTransactionsByBookingId(bookingId: string): Promise<Transaction[]> {
    const { data, error } = await supabaseAdmin
      .from('transactions')
      .select('*')
